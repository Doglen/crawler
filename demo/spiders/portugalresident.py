...
class portugalresidentSpider(scrapy.Spider):
    ...
    def __init__(self, time=None, *args, **kwargs):
        super(portugalresidentSpider, self).__init__(*args, **kwargs) # 将这行的DemoSpider改成本类的名称
        self.time = time
    ...
    def parse_category(self, response):
        ...
        if Util.format_time3(time_adjustment(last_time[-4])) >= int(self.time):
            if soup.find("div", class_="page-nav td-pb-padding-side").select("a")[-1].select_one("i.td-icon-menu-right"):
